...
public class PassphraseValidator {
    ...
    private static final String list = "oaoe rxeq vssdqtu xrk cjv yaoqp loo\n" +
    ...
    public static boolean isValid(final String passphrase) {
        final String[] splitted = passphrase.split(" ");
        boolean ok = true;
        
        for (int i = 0; i < splitted.length; i++) {
            int counter = 0;
            for (String s : splitted) {
                if (splitted[i].equals(s)) counter++;
            }
            ok = ok && counter == 1;
        }
    ...
    public static boolean isAnagram(final String value1, final String value2) {
        boolean isAnagram = true;
        final ArrayList<String> v1 = new ArrayList<String>();
        final ArrayList<String> v2 = new ArrayList<String>();
        final String[] splitted1 = value1.split("");
        final String[] splitted2 = value2.split("");
        for (String v : splitted1) {
            v1.add(v);
        }
        for (String v : splitted2) {
            v2.add(v);
        }
        
        return v1.containsAll(v2) && v1.size() == v2.size();
    }
    
    
    public static boolean isValid2(final String passphrase) {
        final String[] splitted = passphrase.split(" ");
        boolean ok = true;
